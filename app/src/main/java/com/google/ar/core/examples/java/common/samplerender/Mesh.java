...
//      downloadMaskImage();
  ...
  public void makeBuffer(SampleRender render, float[] vertices, float[] normals, int[] indices){
    for (int i = 0; i < vertexBuffers.length; i++) this.vertexBuffers[i].close();
    this.indexBuffer.close();

    // Add program to OpenGL environment.
    ByteBuffer byteBuffer = ByteBuffer.allocateDirect(vertices.length * 4);
    byteBuffer.order(ByteOrder.nativeOrder());
    FloatBuffer localCoordinates = byteBuffer.asFloatBuffer();
    localCoordinates.put(vertices);
    localCoordinates.position(0);

    ByteBuffer byteBufferNormal = ByteBuffer.allocateDirect(normals.length * 4);
    byteBufferNormal.order(ByteOrder.nativeOrder());
    FloatBuffer localNormals = byteBufferNormal.asFloatBuffer();
    localNormals.put(normals);
    localNormals.position(0);

    VertexBuffer[] vertexBuffers = {
            new VertexBuffer(render, 3, localCoordinates),
            new VertexBuffer(render, 3, localNormals),
    };

    byteBuffer = ByteBuffer.allocateDirect(indices.length * 4);
    byteBuffer.order(ByteOrder.nativeOrder());
    IntBuffer vertexIndices = byteBuffer.asIntBuffer();
...
//    GLES30.glGenVertexArrays(1, vertexArrayId, 0);
  ...
  public void makeBuffer(SampleRender render, float[] vertices, int[] indices){
    for (int i = 0; i < vertexBuffers.length; i++) this.vertexBuffers[i].close();
    this.indexBuffer.close();
...
//    GLES30.glGenVertexArrays(1, vertexArrayId, 0);
  ...
  /**
   * Download flatten rendered image, must call after draw
   * @return color byte 1D array
   */
  public byte[] downloadMaskImage() {
    final int[] source = new int[width * height];
    final IntBuffer sourceBuffer = IntBuffer.wrap(source);
    sourceBuffer.position(0);

    GLES30.glReadPixels(0, 0, width, height, GLES30.GL_RGBA, GLES30.GL_UNSIGNED_BYTE, sourceBuffer);
    GLES30.glFinish();
...
//    long start = System.currentTimeMillis();
    ...
    for(int i=0, k=0; i<height; i++, k++)
    {
      ...
      {
//        final int green = (source[i*width+j] & 0x0000FF00) >> 8;
        final int green = (source[i*width+j] & 0xFF000000) >> 24;
        if (green != 0)
          maskImage[(height-k-1)*width+j] = (byte) 255;
        else
          maskImage[(height-k-1)*width+j] = (byte) 0;
      }
    }
...
//    Log.e("Image", String.valueOf(end - start));
  ...
  /**
   * Download color image, must call after draw
   * @return color ARGB int 1D array
   */
  public int[] downloadColorImage(){
    final int[] source = new int[width * height];
    final IntBuffer sourceBuffer = IntBuffer.wrap(source);
    sourceBuffer.position(0);
    ...
    int[] colorImage = new int[width * height];
    for(int i=0, k=0; i<height; i++, k++) {
      for(int j=0; j<width; j++) {
        int pix = source[i * width + j];
        int pb = (pix >> 16) & 0xff;
        int pr = (pix << 16) & 0x00ff0000;
        int pix1 = (pix & 0xff00ff00) | pr | pb;
        colorImage[(height-k-1)*width+j] = pix1;
      }
    }
    return colorImage;
